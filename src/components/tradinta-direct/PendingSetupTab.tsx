...
export function PendingSetupTab() {
  const { user } = useUser();
  ...
  const productsQuery = useMemoFirebase(() => {
    ...
    return query(
      ...
      where('tradintaDirectStatus', '==', 'pending_setup')
    );
  }, [firestore, user]);
  ...
  const renderTableRows = () => {
    ...
    return products.map((product) => (
      <TableRow key={product.id}>
        ...
        <TableCell>
            {`KES ${product.variants?.[0]?.price?.toLocaleString() || 'N/A'}`}
        </TableCell>
        <TableCell>
  ...
  return (
    <Card>
        <CardHeader>
            ...
            <CardDescription>These products require B2C pricing and inventory setup before they go live on Tradinta Direct.</CardDescription>
        </CardHeader>
        <CardContent>
            <Table>
                <TableHeader>
